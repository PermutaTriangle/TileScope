...
@pytest.fixture
def fusion_with_req(tiling_with_req):
...
def test_requirements_fuse_counters(row_fusion, fusion_with_req):
...
def test_can_fuse_set_of_gridded_perms(row_fusion, col_fusion, col_fusion_big):
...
def test_is_valid_count(row_fusion):
    assert row_fusion._is_valid_count(3, Obstruction(Perm((1, 0)), ((0, 0), (0, 0))))
    assert row_fusion._is_valid_count(3, Obstruction(Perm((1, 0)), ((0, 0), (1, 0))))
    assert row_fusion._is_valid_count(2, Obstruction(Perm((1, 0)), ((1, 1), (1, 0))))
    assert not row_fusion._is_valid_count(1, Obstruction(Perm((1, 0)), ((1, 0), (1, 0))))

def test_point_in_fuse_region(row_fusion, col_fusion_big, gp1, gp2):
...
def test_fusable(row_fusion, col_fusion, col_fusion_big, fusion_with_req):
...
def test_fused_tiling(row_fusion, col_fusion, col_fusion_big, fusion_with_req):
